...
def main():
    DRAE_dictionaries_folder = "../dictionaries/DRAE/"
    DALLA_dictionary_folder = "../dictionaries/DALLA/"
    ...
    for dictionary in tqdm(dictionay_xhtml_pages):
        ...
        exception_indexes = [index for index in range(len(document_meaning_number_positions)) if document_meaning_number_positions[index] == None]
    ...
    print(len(final_document_headwords))
    print(len(final_document_entries))
    exit()

    # Treat exceptions and append everything
    # Quitar mas tarde, codigo por si hay que establecer los tipos
    #locs_index = get_locs(document_exception_entries)
    #parts_index = get_parts(document_exception_entries)
    #locs_exception_headwords = [document_exception_headwords[index] for index in locs_index]
    #parts_exception_headwords = [document_exception_headwords[index] for index in parts_index]
    #locs_exception_entries = [document_exception_entries[index] for index in locs_index]
    #print(locs_exception_entries)
    #parts_exception_entries = [document_exception_entries[index] for index in parts_index]
    ...
    # Guardar la información del csv en sqlite3
